...
`useVoice` フックが `POST /api/v1/speech` を呼び出し、OpenAI TTS が生成した MP3 を再生する。

- ページ滞在中は同じフレーズの音声を `speechCache` (Map) にメモリキャッシュし、API 通信を 1 回に抑える
- API 側では Cloudflare R2 にキャッシュされるため、2 回目以降は OpenAI を呼ばずに即返却される
- フェッチ中に別フレーズへ遷移した場合は `AbortController` でリクエストをキャンセルする
